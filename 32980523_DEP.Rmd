...
reviewsNum.mel <- all.reviews.enriched %>% 
                  filter(nb_group =="Melbourne") %>% 
                  filter(nb_cleansed ==c("Melbourne","Yarra Ranges","Port Phillip","Yarra","Stonnington")) %>% 
                  group_by(date,nb_cleansed) %>%
                  summarise(number = n())
# plot popularity

ggplot(reviewsNum.mel, aes(date, number)) +
           geom_point(na.rm=TRUE, color ="#002B5B" alpha=0.5) +geom_smooth(color = "#FF5A5F")+
  ggtitle("Evolution of Listing's Popularity",
          subtitle = "Number of Reviews") +
  labs(x = "Year", y = "listings with reviews") +
  theme(plot.title = element_text(face = "bold")) +
...
ggplot(reviewsNum.top5, aes(date, number)) +
           geom_point(na.rm=TRUE, alpha=0.5,mapping=aes(color=nb_group)) +geom_smooth(color = "white")+
  ggtitle("Evolution of Listing's Popularity",
          subtitle = "Number of Reviews") +
  labs(x = "Year", y = "listings with reviews") +
  theme(plot.title = element_text(face = "bold")) +
...
reviews_summary <- all.listings.final %>% 
