...
```{r}
head(all.reviews.enriched %>% filter(listing_id =="9835"))
```
...
```{r}
# first we will need to combine the 

reviewsNum <- all.reviews %>% group_by(date = all.reviews$date) %>% summarise(number = n())

ggplot(reviewsNum, aes(date, number)) +
           geom_point(na.rm=TRUE, color = "#007A87", alpha=0.5) +geom_smooth(color = "#FF5A5F")+
  ggtitle("Evolution of L",
