...
#preparation of data
#melbourne
all.reviews.mel <- all.reviews.enriched %>% 
  filter(last_scraped.new=="Jun-2022",
         nb_group=="Melbourne") %>%
  group_by(longitude,latitude,listing_id,nb_cleansed) %>%
  summarise(count = n())
```
...
all.reviews.map %>%
  ...
  ggplot(aes(x=longitude, y=latitude, size=count, fill=nb_cleansed)) +
    geom_point(alpha=0.5, shape=21, color="black") +
    scale_size(range = c(.1, 24), name="Number of unique reviews") +
    scale_fill_viridis(discrete=TRUE, guide=FALSE, option="A") +
    theme_ipsum() +
    theme(legend.position = "none")
#Tassie
...
ggmap(get_googlemap(center=c(lon=144.94,lat=-37.84),
                                     zoom = 10,maptype ='terrain',
                                     color = 'color')) + 
  geom_point(data=ggmap.meldata,aes(x=longitude,y=latitude,color=count),size=1,alpha=0.75)+
